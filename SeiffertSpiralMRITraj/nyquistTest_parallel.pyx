...
def process_3d_array_parallel(np.ndarray[DTYPE_f, ndim = 4] gridPoints, np.ndarray nyquistPoints, np.ndarray[DTYPE_i, ndim = 3] dsphere, double dk_nyquist):
    ...
    # Separate lists for each thread's results
    cdef Nthreads = 5
    cdef list requiredPoints_threads = [ [] for _ in range(0, Nthreads) ]
    ...
    with nogil, parallel():
        ...
        for i in prange(1, gridPoints.shape[0] - 1):
                    ...
                    for m in range(3):
                        ...
                        if (dsphere[i_n, j_n, p_n] > 0):# this grid point is in the sphere and can be checked 
                            ...
                            if not(nyquistResult):
                                ...
                                requiredPoints_threads[cython.parallel.threadid()].append(list(midPoint))
                    
    #return requiredPoints
    #requiredPoints =  [point for sublist in requiredPoints_threads for point in sublist]
    #return requiredPoints
    cdef int result = 1
    return result
